...
<?php include_once __DIR__ . '/../../layouts/header_paciente.php'; ?>
...
<form id="formFotoPerfilPaciente"
      class="form-foto"
      action="<?= BASE_URL ?>/paciente/guardar-foto-perfil"
      method="POST"
      enctype="multipart/form-data">

<input type="hidden" name="id" value="<?= $perfil['id'] ?>">
<input type="hidden" name="accion" value="actualizarFotoPaciente">
...
<img class="pacienteImg"
     src="<?= BASE_URL ?>/public/uploads/pacientes/<?= $perfil['foto'] ?>"
     alt="<?= $perfil['paciente_nombre'] ?>"
...
<h6 style="font-weight:600;">@<?= $perfil['paciente_nombre'] ?></h6>
...
<?= $perfil['paciente_nombre'] ?>, <?= $perfil['apellidos'] ?>
</span>
</div>
...
<form action="<?= BASE_URL ?>/paciente/guardar-configuracion-usuario"
      method="POST">

<input type="hidden" name="id" value="<?= $perfil['id'] ?>">
<input type="hidden" name="accion" value="actualizarInfoPersonalPaciente">
...
<form action="<?= BASE_URL ?>/paciente/cambiar-contrasena"
      method="POST">

<input type="hidden" name="id" value="<?= $perfil['id'] ?>">
<input type="hidden" name="accion" value="actualizarContrasenaPaciente">
...
document.getElementById("fotoPaciente").addEventListener("change", function () {
    ...
    if (this.files && this.files[0]) {
        ...
        reader.onload = function (e) {
            document.querySelector(".pacienteImg").src = e.target.result;
        };
        reader.readAsDataURL(this.files[0]);

        document.getElementById("formFotoPerfilPaciente").submit();
    }
