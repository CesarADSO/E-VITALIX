...
<body>
    ...
    <div class="header">
        <div class="header-text">
            ...
            <p style="color: #1c1c1c; margin-top: 10px;">
                <strong>Reporte Oficial:</strong> Consulta Médica número <?= $id_consulta ?><br>
    ...
    <h2 class="titulo-seccion">REGISTRO CLÍNICO DE ATENCIONES CONSULTA MÉDICA NÚMERO <?= $id_consulta ?></h2>
    ...
    <?php else: ?>
            <div class="bloque-cita">
                ...
                <div class="cita-header">
                    <strong>Fecha:</strong> <?= date('d/m/Y', strtotime($consulta['fecha_consulta'])) ?> |
                    <strong>Médico Tratante:</strong> Dr(a). <?= $consulta['nombre_especialista'] ?> <?= $consulta['apellido_especialista'] ?> (<?= $consulta['especialidad'] ?>)
                </div>
                ...
                <table>
                    <tbody>
                        <tr>
                            ...
                            <td style="width: 10%;"><?= $consulta['presion_sistolica'] ?>/<?= $consulta['presion_diastolica'] ?></td>
                            <th style="width: 15%;">Temp.</th>
                            <td style="width: 10%;"><?= $consulta['temperatura'] ?>°C</td>
                            <th style="width: 15%;">F. Cardíaca</th>
                            <td style="width: 10%;"><?= $consulta['frecuencia_cardiaca'] ?> lpm</td>
                            <th style="width: 15%;">F. Respiratoria</th>
                            <td style="width: 10%;"><?= $consulta['frecuencia_respiratoria'] ?> rpm</td>
                        </tr>
                    </tbody>
                </table>

                <h2 class="text-box-label">Diagnóstico Clínico</h2>
                <p class="text-box"><?= $consulta['diagnostico'] ?></p>

                <h2 class="text-box-label">Plan de Tratamiento / Órdenes Médicas</h2>
                <p class="text-box"><?= $consulta['tratamiento'] ?> <?= $consulta['orden_medica'] ?></p>

                <?php if (!empty($consulta['nombre_medicamento'])): ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                <td><?= $consulta['nombre_medicamento'] ?></td>
