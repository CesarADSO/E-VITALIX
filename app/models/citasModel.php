...
class CitasModel
{
    ...
    public function aceptarCita($id)
    {
        try {
            ...
            $obtenerDatos = "SELECT especialistas.nombres, especialistas.apellidos, especialidades.nombre AS nombre_especialidad, consultorios.nombre AS nombre_consultorio, ciudades.nombre AS ciudad, consultorios.direccion, agenda_slot.fecha, agenda_slot.hora_inicio, agenda_slot.hora_fin, usuarios.email FROM citas INNER JOIN agenda_slot ON citas.id_agenda_slot = agenda_slot.id INNER JOIN especialistas ON agenda_slot.id_especialista = especialistas.id INNER JOIN especialidades ON especialistas.id_especialidad = especialidades.id INNER JOIN pacientes ON citas.id_paciente = pacientes.id INNER JOIN usuarios ON pacientes.id_usuario = usuarios.id INNER JOIN consultorios ON agenda_slot.id_consultorio = consultorios.id INNER JOIN ciudades ON consultorios.id_ciudad = ciudades.id WHERE citas.id = :id_cita";
