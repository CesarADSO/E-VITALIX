...
class EspecialistaDashboard
{
    ...
    public function obtenerUltimasCitasPendientes($id_especialista) {
        try {
            $consulta = "SELECT citas.id AS id_cita, pacientes.id AS id_paciente, pacientes.nombres AS nombre_paciente, pacientes.apellidos AS apellido_paciente, agenda_slot.fecha, citas.estado_cita FROM citas INNER JOIN agenda_slot ON citas.id_agenda_slot = agenda_slot.id INNER JOIN pacientes ON citas.id_paciente = pacientes.id WHERE agenda_slot.id_especialista = :id_especialista AND citas.estado_cita IN ('PENDIENTE', 'CONFIRMADA') ORDER BY agenda_slot.fecha ASC LIMIT 5";
